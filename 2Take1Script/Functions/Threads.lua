...
function Threads.Clearpeds(Data)
    ...
    for i = 1, #Peds do
        if not ped.is_ped_a_player(Peds[i]) and entity.get_entity_coords(Peds[i]):magnitude(player.get_player_coords(player.player_id())) < Distance then
            utility.clear(Peds[i])
            done  = done + 1
        end
...
function Threads.Clearvehicles(Data)
    ...
    for i = 1, #Vehicles do
        ...
        for j = 1, #PersonalVehicles do
            if Vehicles[i] == PersonalVehicles[j] then
                isPV = true
            end
        end
    
        if not isPV and not ped.is_ped_a_player(vehicle.get_ped_in_vehicle_seat(Vehicles[i], -1)) and entity.get_entity_coords(Vehicles[i]):magnitude(player.get_player_coords(player.player_id())) < Distance then
            utility.clear(Vehicles[i])
            done = done + 1
        end
...
function Threads.Clearobjects(Data)
    ...
    for i = 1, #Objects do
        if entity.get_entity_coords(Objects[i]):magnitude(player.get_player_coords(player.player_id())) < Distance then
