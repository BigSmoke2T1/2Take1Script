...
function utility.valid_modder(PlayerID)
    if player.is_player_valid(PlayerID) then

        if PlayerID ~= player.player_id() and player.can_player_be_modder(PlayerID) and not player.is_player_modder(PlayerID, -1) then
            return true
        end
...
function utility.print_table(Source)
...
function utility.GetVehicleMods(Vehicle)
    ...
    VehicleData.pearlcolor = vehicle.get_vehicle_pearlecent_color(Vehicle)
    VehicleData.wheelcolor = vehicle.get_vehicle_wheel_color(Vehicle)
    ...
    VehicleData.wheeltype = vehicle.get_vehicle_wheel_type(Vehicle)

    VehicleData.platetype = vehicle.get_vehicle_number_plate_index(Vehicle)
...
function utility.SetVehicleMods(Vehicle, VehicleData, Extra)
    ...
    vehicle.set_vehicle_extra_colors(Vehicle, VehicleData.pearlcolor, VehicleData.wheelcolor)
    ...
    vehicle.set_vehicle_wheel_type(Vehicle, VehicleData.wheeltype)

    vehicle.set_vehicle_bulletproof_tires(Vehicle, true)

    vehicle.set_vehicle_number_plate_index(Vehicle, VehicleData.platetype)
    vehicle.set_vehicle_number_plate_text(Vehicle, VehicleData.platetext)
...
function utility.clear(Data, Time)
    if Data and type(Data) == 'table' then
        ...
        entity.set_entity_visible(Data, false)
